...
const Products = () => {
	...
	const sortedProducts = useMemo(() => {
		...
		list.sort((a, b) => {
			...
			if (sortBy === "name-asc") return String(a?.name || "").localeCompare(String(b?.name || ""), undefined, { sensitivity: "base" });
	...
	useEffect(() => {
		dispatch(getAllProductsAction({ page, limit, sortBy }));
		...
		<div>
			...
			<div className="w-full mx-auto">
				{sortedProducts.length === 0 ? (
					...
					<div className={"w-full gap-4 max-w-10/10 mx-auto grid grid-cols-1 place-content-center md:grid-cols-2 lg:grid-cols-4"}>
							...
							<div key={product._id} className={"border relative p-4 rounded-lg shadow-md"}>
								<div className={"gap-2"}>
									<button className={"w-full text-left cursor-pointer"} onClick={() => navigate(`/products/${product._id}`)}>
										<img src={product.images} alt={product.name} className={"w-full h-48 object-cover rounded-md mb-4"} />
								...
								{product.isAvailable ? (
									<button onClick={(e) => { e.stopPropagation(); handleAddToCart(product._id); }} className="btn m px-10 cursor-pointer bg-yellow-500 rounded-xl p-1 btn-primary">
										Add to cart
									</button>
								) : (
