				...
				<div className={"w-full   gap-4 max-w-10/10 mx-auto grid grid-cols-1 place-content-center md:grid-cols-2" +
					...
					{sortedProducts?.map?.((product)=>{
						return (
							<div key={product._id} className={"border relative p-4 rounded-lg shadow-md"}>
											<div className={"gap-2"}>
												...
												<button className={"w-full text-left cursor-pointer"} onClick={()=>console.log("button Clicked")}>
						...
						<p><strong>Price:</strong> ₹{product.price}</p>
								{
									product.isAvailable?<button onClick={()=>{handleAddToCart(product._id)}} className="btn m px-10 cursor-pointer  bg-yellow-500 rounded-xl p-1   btn-primary">Add to cart</button>
